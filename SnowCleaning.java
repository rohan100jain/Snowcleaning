...
class SnowCleaning {
  ...
  private boolean isLocationSnowed(int row, int col, int[] snowFalls) {
    int K = snowFalls.length / 2;
    for (int i = 0; i < K; ++i) {
      int row1 = snowFalls[2 * i];
      int col1 = snowFalls[2 * i + 1];
      if (row == row1 && col == col1) return true;
    }
    return false;
  }
  ...
  private MoveData nearestNotRequiredHelper(int start_row, int start_col,
    ...
    if (worker != -1) {
      if (!isLocationSnowed(start_row, start_col, snowFalls)) {
  ...
  private MoveData nearestNotRequiredWorker(int row, int col,
      ArrayList<Integer> moved_workers,int[] snowFalls) {
    if (col != 0) {
      MoveData move_data = nearestNotRequiredHelper(row, col - 1, row, col,
          "R", moved_workers, snowFalls);
      if (move_data != null) {
        return move_data;
      }
    }
  ...
  public String[] nextDay(int[] snowFalls) {
    ...
    for (int i = 0; i < K; ++i) {
      ...
      if (!hasWorker) {
        MoveData move_data = nearestNotRequiredWorker(row, col, moved_workers,
            snowFalls);
        if (move_data != null) {
